...
export default function JobOpeningsCMS() {
  ...
  function mapJobToRow(job) {
    const status = job.status || "Draft / Unlisted";
    const statusStyle = status === "Active / Open" ? "bg-emerald-50 text-emerald-600" : status === "Draft / Unlisted" ? "bg-stone-100 text-stone-500" : "bg-amber-50 text-amber-600";
    return { ...job, id: job._id || job.id, icon: Briefcase, title: job.title || "Untitled Job", format: job.format || "Full-Time Enterprise Requisition", department: job.department || "", location: job.location || "", seniority: job.seniority || "", pitch: job.pitch || "", compMin: job.compMin ?? 0, compMax: job.compMax ?? 0, hiringManager: job.hiringManager || MANAGERS[0], recruiter: job.recruiter || RECRUITERS[0], urgency: job.urgency || "immediate", tags: job.tags || [], checks: job.checks || [true,true,true], syndication: job.syndication || [true,true,true], status, statusStyle, applicants: `${job.applicants ?? 0} Applicants`, sub: job.sub || (status === "Draft / Unlisted" ? "Draft Review" : "Awaiting Review"), subColor: "text-stone-400", posted: job.posted ? new Date(job.posted).toLocaleDateString() : job.createdAt ? new Date(job.createdAt).toLocaleDateString() : "Today", draft: job.draft ?? status === "Draft / Unlisted" };
  }
  ...
  useEffect(() => {
    const loadJobs = async () => {
      try {
        ...
        setRequisitions(jobs.map(mapJobToRow));
      } catch (error) {
        console.error("Failed to load job openings:", error);
        flashToast(error.message || "Failed to load job openings");
      } finally {
  ...
  async function duplicateRow(r) {
    try {
      await createJobOpening({ title: `${r.title} (Copy)`, format: r.format || "Full-Time Enterprise Requisition", department: r.department || "", location: r.location || "", seniority: r.seniority || "", pitch: r.pitch || "", compMin: Number(r.compMin) || 0, compMax: Number(r.compMax) || 0, hiringManager: r.hiringManager || MANAGERS[0], recruiter: r.recruiter || RECRUITERS[0], urgency: r.urgency || "immediate", tags: r.tags || [], checks: r.checks || [true,true,true], syndication: r.syndication || [true,true,true], status: "Draft / Unlisted", applicants: 0, sub: "Draft Review", draft: true });
      const jobs = await getJobs();
      setRequisitions(jobs.map(mapJobToRow));
      flashToast("Requisition duplicated as draft");
    } catch (error) {
      console.error("Duplicate Job Error:", error);
      flashToast(error.message || "Failed to duplicate requisition");
    }
  }
  ...
  async function deleteRow(r) {
    ...
    try {
      await deleteJobOpening(r.id);
      setRequisitions((list) => list.filter((x) => x.id !== r.id));
      if (editingId === r.id) { setForm(emptyForm()); setEditingId(null); }
      flashToast("Requisition deleted");
    } catch (error) {
      console.error("Delete Job Error:", error);
      flashToast(error.message || "Failed to delete requisition");
    }
  }

  async function saveAs(status) {
    if (!form.title.trim() || form.title.trim() === "Untitled Requisition") { flashToast("Add a title before saving"); return; }
    const payload = { title: form.title.trim(), format: form.format || "Full-Time Enterprise Requisition", department: form.department || "", location: form.location || "", seniority: form.seniority || "", pitch: form.pitch || "", compMin: Number(form.compMin) || 0, compMax: Number(form.compMax) || 0, hiringManager: form.hiringManager || MANAGERS[0], recruiter: form.recruiter || RECRUITERS[0], urgency: form.urgency || "immediate", tags: form.tags || [], checks: form.checks || [true,true,true], syndication: form.syndication || [true,true,true], status, applicants: editingId ? undefined : 0, sub: status === "Draft / Unlisted" ? "Draft Review" : "Awaiting Review", draft: status === "Draft / Unlisted" };
    Object.keys(payload).forEach((key) => { if (payload[key] === undefined) delete payload[key]; });
    try {
      if (editingId) {
        await updateJobOpening(editingId, payload);
      } else {
        const result = await createJobOpening(payload);
      ...
      flashToast(error.message || "Failed to save job opening");
    }
  }
